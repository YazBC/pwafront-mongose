...
export async function create(req, res) {
  const { title, description = "", status = "Pendiente", clienteId, fechaInicio, fechaVencimiento } = req.body;
...
export async function update(req, res) {
  const { id } = req.params;
  // 👇 Agregamos fechaInicio y fechaVencimiento aquí
  const { title, description, status, fechaInicio, fechaVencimiento } = req.body;
  ...
  const task = await Task.findOneAndUpdate(
    ...
    { title, description, status, fechaInicio, fechaVencimiento }, // 
    { new: true }
  );
  ...
  try {
    ...
    for (const t of tasks) {
      ...
      if (!doc) {
        ...
        // 👇 Para cuando se edita offline
        if (t.fechaInicio !== undefined) doc.fechaInicio = t.fechaInicio;
        if (t.fechaVencimiento !== undefined) doc.fechaVencimiento = t.fechaVencimiento;
